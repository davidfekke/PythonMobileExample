...
class MasterViewController: UITableViewController {
    ...
    // MARK: Call Python server
    
    func setObjectFromPythonServer() -> Void {
        let defaultConfiguration = URLSessionConfiguration.default
        let defaultSession = URLSession(configuration: defaultConfiguration)
        if let url = URL(string: "http://localhost:5000/json") {
            (defaultSession.dataTask(with: url) { data, response, error in
                ...
                if let _ = response,
                    ...
                    do {
                        ...
                        NSLog("An error was thrown on getting JSON!")
                    }
                }
            }).resume()
        }
